...
//********************************************************************************
int Pawn::legalMove(int x, int y)
{
    if ((0 <= x && x <= 7) && (0 <= y && y <= 7))
    {
        return 1;
...
int Pawn::getLegalMoves()
{
    ...
    iX = initialRow + 1;    //initalizing x & row for one of the 8 possible moves
    iY = initialColumn + 0; //initalizing y and column for one of the 8 possible moves
    offsets[iCount][0] = iX;
    offsets[iCount][1] = iY;
    if (legalMove(iX, iY))
    {
